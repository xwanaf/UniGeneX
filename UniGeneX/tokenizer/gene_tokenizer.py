...
def random_mask_value(
    ...
    for i in range(len(values)):
        ...
        n_mask = int(len(non_padding_idx) * get_mlm_probability(mask_ratio))
        mask_idx = np.random.choice(non_padding_idx, n_mask, replace=False)
        row[mask_idx] = mask_value
    return torch.from_numpy(values).float()

def get_mlm_probability(mlm_probability) -> float:
        """
        Get the mlm probability for the current step.
        """
        if isinstance(mlm_probability, float):
            return mlm_probability
        elif isinstance(mlm_probability, list):
            # random choose a probability
            return np.random.choice(mlm_probability)
        else:
            raise ValueError(
                "mlm_probability must be a float or a list of floats, "
                f"but got {mlm_probability}."
            )