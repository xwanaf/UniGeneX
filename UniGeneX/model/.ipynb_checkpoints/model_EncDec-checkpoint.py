...
class HeMAE(nn.Module):
    def __init__(
        ...
        decoder_layers = FlashTransformerEncoderLayer(
                    decoder_d_model,
                    nhead,
                    d_hid,
                    dropout,
...
class TransformerAutoEncoderModel(nn.Module):
    def __init__(
        ...
        self.explicit_zero_prob = explicit_zero_prob
        self.norm_scheme = "pre" if pre_norm else "post"
        self.detach_gene_name_emb = detach_gene_name_emb
        ...
        self.encoder = GeneEncoder(ntoken, d_model, padding_idx=vocab[pad_token], do_detach = self.detach_gene_name_emb)
        self.flag_encoder = nn.Embedding(2, d_model)
    ...
    def forward(
        ...
        """
        enc_output = self._encode(
            src, values, src_key_padding_mask
        )
        
        cell_emb = enc_output[:,0,:]
        # Split the result into mu and var components
        # of the latent Gaussian distribution
...
#             self.cell_emb,
        ...
        output["pred"] = pred # batch x seq_len x d_model
                                        #.   (no cell emb at beginning)
