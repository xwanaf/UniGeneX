...
from scgpt_pcpt.loss import masked_mse_loss, masked_relative_error, criterion_neg_log_poisson, criterion_neg_log_poisson_additive, smooth_l1_loss
...
class TransformerVAEModel_pl_SPcontext(pl.LightningModule):
    def __init__(
        self,
        d_model: int,
        nhead: int,
...
#                 self.logger.log_info(f'Subsampling background_adata ...')
    ...
    def build_network(self):
        ...
        if (self.mvc_decoder_type is None) or (self.mvc_decoder_type == 'Inner'):
            ...
            self.N_MidLayer = MidLayer_NN(3 * self.d_model, self.d_model, dropout=self.dropout, cell_emb_concat = True)
            if self.fast_transformer_backend == "flash":
                ...
                ################ NN related network ################
                self.pre_patch_norm = nn.LayerNorm(self.d_model)
                N_mvc_layers = FlashTransformerEncoderLayer_FiLM(
                            self.d_model,
                            self.nhead,
                            self.d_hid,
                            self.dropout,
                ...
                self.N_mvc_decoder = FlashTransformerEncoder(N_mvc_layers, 1)
                
                NN_emb_layers = FlashTransformerEncoderLayer_FiLM(
                            self.d_model,
                            self.nhead,
                            self.d_hid,
                            self.dropout,
                ...
                self.NN_emb = FlashTransformerEncoder(NN_emb_layers, 1)
                
                self.gene_mask_decoder = ExprDecoder(
                    self.d_model,
                    explicit_zero_prob=self.explicit_zero_prob,
                )
            elif self.fast_transformer_backend == "normal":
            ...
            self.mvc_Delta_expr_decoder = ExprDecoder(
                                                self.d_model,
                                                explicit_zero_prob=self.explicit_zero_prob,
                                            )
    ...
    def on_validation_epoch_start(self):
        ...
        self.sample_cells_delta_test = []
        self.sample_cell_embs_test = []
        self.sample_N_cell_embs_test = []
    ...
    def validation_step(self, batch, batch_idx, dataloader_idx = 0):  
        ...
        if dataloader_idx == 0:
            ...
            cell_id, X_samples_delta, X_sample_gene_mask, X_samples, X_sample_N_cell_emb, celltype_column = batch['id'], output_dict['recon_delta_output_raw'].detach(), output_dict['gene_mask'].detach(), output_dict['recon_output'].detach(), output_dict['N_cell_emb'].detach() , batch['celltype_code'].detach()
            out_dict = {
                'cell_id': cell_id, 
                'sample_cells': X_samples, 
                'sample_cells_delta': X_samples_delta, 
                'sample_cell_embs': X_sample_gene_mask, 
                'sample_N_cell_embs': X_sample_N_cell_emb, 
    ...
    def on_validation_batch_end(self, batch_parts, batch, batch_idx, dataloader_idx = 0): # batch_parts already combined the results of all gpus!! great
        ...
        if dataloader_idx == 1: # test
            ...
            self.sample_cells_delta_test.extend(batch_parts['sample_cells_delta'].tolist())
            self.sample_cell_embs_test.extend(batch_parts['sample_cell_embs'].tolist())
            self.sample_N_cell_embs_test.extend(batch_parts['sample_N_cell_embs'].tolist())
    ...
    def on_validation_epoch_end(self): 
        ...
        if len(self.CellTypeMapping_df) == 2:
            ...
            self.sample_cells_delta_test = torch.Tensor(self.sample_cells_delta_test).to(self.device)
            self.sample_cell_embs_test = torch.Tensor(self.sample_cell_embs_test).to(self.device)
            self.sample_N_cell_embs_test = torch.Tensor(self.sample_N_cell_embs_test).to(self.device)
        ...
        def plot_cell_sample(cell_id, celltype_column, sample_cells, sample_cell_embs, CellTypeMapping_df, dataset_idx = None, suffix = None, sp_adata = None, testDS = False, sample_cells_delta = None, sample_N_cell_embs = None):
            ...
            if sample_cells_delta is not None:
                sample_cells_delta = gather_list(sample_cells_delta)
                sample_N_cell_embs = gather_list(sample_N_cell_embs)
            ...
            self.logger.log_info(f'sample_cell_adata X max: {sample_cells_adata.X.max()}.')

            if sample_cells_delta is not None:
                X_samples_delta = torch.Tensor(sample_cells_delta).cpu().numpy()
                sample_cells_delta_adata = sc.AnnData(X = X_samples_delta)
                sample_cells_delta_adata.X = csr_matrix(sample_cells_delta_adata.X)
                sample_cells_delta_adata.obs['cell_type'] = celltype_column
                sample_cells_delta_adata.obs['cell_id'] = cell_id
                sample_cells_delta_adata.obs['dataset_idx'] = ['dataset_' + str(_) for _ in dataset_idx]
                self.logger.save_h5ad(sample_cells_delta_adata, f'{self.current_epoch}_sample_cell_delta_adata_{suffix}')
                self.logger.log_info(f'Writing {self.current_epoch}_sample_cell_delta_adata.h5ad in {self.logger._save_dir}.')
                self.logger.log_info(f'sample_cell_delta_adata shape: {sample_cells_delta_adata.shape}.')
                self.logger.log_info(f'sample_cell_delta_adata X max: {sample_cells_delta_adata.X.max()}.')

                X_sample_N_cell_emb = torch.Tensor(sample_N_cell_embs).cpu().numpy()
                sample_N_cell_emb_adata = sc.AnnData(X = X_sample_N_cell_emb)
                sample_N_cell_emb_adata.X = csr_matrix(sample_N_cell_emb_adata.X)
                sample_N_cell_emb_adata.obs['cell_type'] = celltype_column
                sample_N_cell_emb_adata.obs['cell_id'] = cell_id
                sample_N_cell_emb_adata.obs['dataset_idx'] = ['dataset_' + str(_) for _ in dataset_idx]
                self.logger.save_h5ad(sample_N_cell_emb_adata, f'{self.current_epoch}_sample_N_cell_emb_adata_{suffix}')
                self.logger.log_info(f'Writing {self.current_epoch}_sample_N_cell_emb_adata.h5ad in {self.logger._save_dir}.')
                self.logger.log_info(f'sample_N_cell_emb_adata shape: {sample_N_cell_emb_adata.shape}.')
                self.logger.log_info(f'sample_N_cell_emb_adata X max: {sample_N_cell_emb_adata.X.max()}.')
            ...
            self.logger.log_info(f'sample_cell_emb_adata X max: {sample_cell_emb_adata.X.max()}.')
            ...
            if sp_adata is None:
                ...
                self.plotsampleumap_single(
                    sample_adata = sample_cells_delta_adata,
                    metric = 'cosine',
                    prefix = f'cell_delta_preds_{suffix}',
                    save_dir = self.logger._save_dir / 'sample_img',
                    step = self.current_epoch
                )
                self.plotsampleumap_single(
                    sample_adata = sample_N_cell_emb_adata,
                    metric = 'cosine',
                    prefix = f'N_cell_embs_{suffix}',
                    save_dir = self.logger._save_dir / 'sample_img',
                    step = self.current_epoch
                )
                sample_cell_emb_adata = sample_cell_emb_adata[~sample_cell_emb_adata.obs['cell_id'].duplicated(keep='first')]
                cell_id_index = sample_cell_emb_adata.obs['cell_id'].astype(int)
                sample_cell_emb_adata.obsm['X_spatial'] = sp_adata[cell_id_index].obsm['X_spatial']
                
                sample_cells_adata = sample_cells_adata[~sample_cells_adata.obs['cell_id'].duplicated(keep='first')]
                cell_id_index = sample_cells_adata.obs['cell_id'].astype(int)
                sample_cells_adata.obsm['X_spatial'] = sp_adata[cell_id_index].obsm['X_spatial']

                sample_cells_delta_adata = sample_cells_delta_adata[~sample_cells_delta_adata.obs['cell_id'].duplicated(keep='first')]
                cell_id_index = sample_cells_delta_adata.obs['cell_id'].astype(int)
                sample_cells_delta_adata.obsm['X_spatial'] = sp_adata[cell_id_index].obsm['X_spatial']

                sc.pp.highly_variable_genes(sample_cell_emb_adata, flavor='seurat', n_top_genes=10)
                # sc.pl.highly_variable_genes(sample_cell_emb_adata)
                show_gene = sample_cell_emb_adata.var_names[sample_cell_emb_adata.var['highly_variable']][0]
                # show_gene = sample_cell_emb_adata.var_names[np.argmax(sample_cell_emb_adata.X.sum(0))]
                show_gene_name = self.vocab.lookup_token(int(show_gene))
                self.logger.log_info(f' Show gene {show_gene_name} at epoch {self.current_epoch}')

                ####### show
                save_dir = self.logger._save_dir / 'sample_img'
                step = self.current_epoch
                
                for adata_vis, save_name in zip([sample_cell_emb_adata, sample_cells_adata, sample_cells_delta_adata], ['gene_mask', 'expr', 'delta_expr']):
                    fig, axs = plt.subplots(1, 2, figsize=(20, 10), dpi = 150, gridspec_kw={'wspace': .2, 'hspace': 0.05})
                    sc.pl.embedding(
                        adata_vis, basis = 'X_spatial', color=show_gene, 
                        size=20, frameon=False, show=False, ax=axs[0], 
                        cmap = 'viridis'
                    )
                    axs[0].legend(bbox_to_anchor=(1., 1.0))
                    axs[0].set_title(show_gene_name)
                    axs[1].remove()
                    
    
                    plt.savefig(save_dir / f"{step}_batch_{show_gene_name}_{save_name}.png") 
                    plt.show()
                    plt.close()
        ...
        if len(self.CellTypeMapping_df) == 2:
            plot_cell_sample(
                ...
                testDS = True,
                sample_cells_delta = self.sample_cells_delta_test,
                sample_N_cell_embs = self.sample_N_cell_embs_test
            )
    ...
    def p_losses_test(self, batch, dec_use_common_gene = True):
        ...
        have_neighbor_mask = data_dict["have_neighbor"]
        self.neighbor_exist = have_neighbor_mask.sum() > 0
        neighbor_index = torch.where(have_neighbor_mask == 1)[0].long().cpu().tolist()
        if self.neighbor_exist > 0:
            N_input_gene_ids = data_dict["N_gene"]
            N_input_values = data_dict["N_expr"]
            N_src_key_padding_mask = N_input_gene_ids.eq(self.vocab[self.pad_token])
            N_mask = (~N_src_key_padding_mask)[:, 1:]
            neighbor_cellid = data_dict["neighbor_cellid"]
            NN_emb_mask = neighbor_cellid.eq(self.vocab[self.pad_token])
        else:
            N_input_gene_ids = None 
            N_input_values = None 
            NN_emb_mask = None 
            N_src_key_padding_mask = None
        ...
        output_dict = self(
            ...
            NN_emb_mask,
            have_neighbor_mask,
            N_src_key_padding_mask = N_src_key_padding_mask,
    ...
    def p_losses(self, batch):
        ...
        self.neighbor_exist = have_neighbor_mask.sum() > 0
        neighbor_index = torch.where(have_neighbor_mask == 1)[0].long().cpu().tolist()
        sc_index = torch.where(have_neighbor_mask == 0)[0].long().cpu().tolist()
        if self.neighbor_exist > 0:
            N_input_gene_ids = data_dict["N_gene"]
            N_input_values = data_dict["N_expr"]
            N_src_key_padding_mask = N_input_gene_ids.eq(self.vocab[self.pad_token])
        ...
        criterion_NN = criterion_neg_log_poisson_additive
        ...
        if self.current_epoch < 5:
            loss = loss_recon = criterion(output_values[sc_index], target_values[sc_index], mask_dec[sc_index])
        ...
        elif self.neighbor_exist:
            output_delta_values = output_dict["recon_delta_output"]
            output_delta_l1_loss = output_dict["recon_delta_output_l1_loss"]

            loss_recon_NN = criterion_NN(
                output_values[neighbor_index], 
                output_delta_values, 
                target_values[neighbor_index], 
                mask_dec[neighbor_index]
            )
            # loss = loss + loss_recon_NN + output_delta_l1_loss
            loss = loss_recon_NN + output_delta_l1_loss
            loss_dict.update({f'{prefix}/recon_NN': loss_recon_NN})
            loss_dict.update({f'{prefix}/recon_NN_l1': output_delta_l1_loss})
        else:
            # loss = torch.tensor([0.0], requires_grad=True).to(self.device)
            loss = loss_recon = criterion(output_values[sc_index], target_values[sc_index], mask_dec[sc_index]) * 0
            loss_dict.update({f'{prefix}/recon': loss_recon})
    ...
    def forward(
        ...
        self.cur_gene_token_embs = dec_src[0, 1:, :]
    ...
    #         KL(N(\mu, \sigma), N(0, 1)) = \log \frac{1}{\sigma} + \frac{\sigma^2 + \mu^2}{2} - \frac{1}{2}
        ...
        #======================== spatial delta x ===========================#
        if have_neighbor_mask.sum() > 0:
            with torch.no_grad():
                N_src = self.encoder(N_src)  # (batch * #neighbor, seq_len, embsize)
                N_values = self.value_encoder(N_values)
                N_total_embs = N_src + N_values
                N_transformer_output = self.transformer_encoder(
                    N_total_embs, src_key_padding_mask=N_src_key_padding_mask
                )
                N_cell_emb = self._get_cell_emb_from_layer(N_transformer_output) # (batch * #neighbor) x embsize
                mu = self.fc_mu(N_cell_emb) # (batch * #neighbor) x d_model
                log_var = self.fc_var(N_cell_emb)
                N_cell_emb = self.reparameterize(mu, log_var)
                neighbor_size = have_neighbor_mask.sum().long().item()
                N_cell_emb = N_cell_emb.view(neighbor_size, -1, self.d_model) # batch x neighbor x d_model

            # ######## transformer
            # N_cell_emb = self.pre_patch_norm(N_cell_emb)
    
            # # cls_token shape: 1 x 1 x d_model
            # cls_tokens = repeat(self.cls_token, "1 1 d -> b 1 d", b=neighbor_size) # batch x 1 x d_model
            # N_cell_emb = torch.cat([cls_tokens, N_cell_emb], dim = 1) # batch x neighbor x d_model
            # NN_emb_out = self.NN_emb(N_cell_emb, src_key_padding_mask = NN_emb_mask)
            # N_cell_emb = NN_emb_out[:, 0] # batch x d_model
            # output["N_cell_emb"] = N_cell_emb  # (batch, seq_len)
            
            ######## take mean
            N_cell_emb_nbr = N_cell_emb[~NN_emb_mask]
            labels = torch.where(NN_emb_mask == False)[0]
            labels = labels.view(labels.size(0), 1).expand(-1, N_cell_emb_nbr.size(1))
            
            unique_labels, labels_count = labels.unique(dim=0, return_counts=True)
            N_cell_emb = torch.zeros_like(unique_labels, dtype=N_cell_emb_nbr.dtype).scatter_add_(0, labels, N_cell_emb_nbr)
            N_cell_emb = N_cell_emb / labels_count.float().unsqueeze(1)
            output["N_cell_emb"] = N_cell_emb  # (batch, seq_len)

            
    
            
            index = torch.where(have_neighbor_mask == 1)[0].long().cpu().tolist()
            src_key_padding_mask = src_key_padding_mask_dec[:, 1:] # dim: batch x gene_len 
            src_key_padding_mask = src_key_padding_mask[index]

            gene_token_embs = self.cur_gene_token_embs.detach()
            sp_cell_emb = output["cell_emb"][index].detach()
            N_dec_output = self.N_MidLayer(
                repeat(gene_token_embs.unsqueeze(0), "1 c d -> b c d", b=have_neighbor_mask.sum().long().item()),
                sp_cell_emb, 
                N_cell_emb.detach()
            ) # batch x seq_len x d_model
            N_dec_output = self.N_mvc_decoder(N_dec_output, src_key_padding_mask = src_key_padding_mask) # have_neighbor x seq_len x d_model
            gene_mask = self.gene_mask_decoder(N_dec_output) # have_neighbor x seq_len 
            gene_mask = gumbel_sigmoid(
                gene_mask['pred'], tau=1, hard=True, threshold=0.5
            )
            
            N_dec_output = self.mvc_Delta_expr_decoder(N_dec_output) # have_neighbor x seq_len 
            
    
            output["gene_mask"] = gene_mask  # (batch, seq_len)
            output["recon_delta_output"] = N_dec_output["pred"] * gene_mask  # (batch, seq_len)
            output["recon_delta_output_raw"] = N_dec_output["pred"] # (batch, seq_len)
            output["recon_delta_output_l1_loss"] = gene_mask.mean()
        #======================== decoder ===========================#
        if (self.mvc_decoder_type is None) or (self.mvc_decoder_type == 'Inner'):
            dec_output = self.mvc_decoder(
                cell_emb,
                repeat(self.cur_gene_token_embs.unsqueeze(0), "1 c d -> b c d", b = cell_emb.shape[0]),
            )
        elif self.mvc_decoder_type == 'LinearConcat':
            dec_output = self.mvc_decoder(
                cell_emb,
...
class MidLayer_NN(nn.Module):
    ...
    def forward(self, x: Tensor, cell_emb=None, N_cell_emb=None) -> Tensor:
        """
        Args:
            x: Tensor, shape [batch_size, seq_len]
        """
        ...
        # expand last dimension
        if self.cell_emb_concat:
            cell_emb = torch.cat([N_cell_emb, cell_emb], dim = -1) # (batch, seq_len, 1024)
            cell_emb = cell_emb.unsqueeze(1)
            cell_emb = cell_emb.repeat(1,x.shape[1],1)
            x = torch.cat([cell_emb, x], dim = -1) # (batch, seq_len, 512*3)
        x = self.activation(self.linear1(x))
        x = self.linear2(x)
        x = self.norm(x)
        return self.dropout(x) 
