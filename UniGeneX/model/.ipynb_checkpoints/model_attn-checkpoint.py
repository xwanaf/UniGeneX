...
# from transformer_utils import TransformerEncoderLayer_returnAttn, TransformerEncoder_returnAttn
...
# from dc_ldm.models.diffusion.plotumap import plotsampledata


from UniGeneX.model.flash_attention import FlashMHA
from UniGeneX.model.transformer_utils import TransformerEncoderLayer_returnAttn, TransformerEncoder_returnAttn
...
class TransformerVAEModel_attn(pl.LightningModule):
    def __init__(
        self,
        d_model: int,
        nhead: int,
...
#                 self.logger.log_info(f'Subsampling background_adata ...')
    ...
    def build_network(self):
        ...
        if (self.mvc_decoder_type is None) or (self.mvc_decoder_type == 'Inner'):
            ...
            if self.fast_transformer_backend == "flash":
                ...
                mvc_layers = TransformerEncoderLayer_returnAttn(
                    self.d_model, self.nhead, self.d_hid, self.dropout, batch_first=True, norm_first = False
                )
                self.mvc_decoder = TransformerEncoder_returnAttn(mvc_layers, self.nlayers_dec)
    ...
    #             attn_weights,
            ...
            if dataloader_idx == 0: # out of sample
                ...
                output_dict = self.p_losses_test(batch, dec_use_common_genes = self.test_use_common_gene)
            ...
            return {'cell_id': cell_id, 'sample_cells': X_samples, 'sample_cell_embs': X_sample_cell_emb, 'celltype_column': celltype_column}


        
        elif self.attn_mode:
            
            dec_use_common_genes = self.valid_use_common_gene if dataloader_idx == 0 else self.test_use_common_gene
            output_dict = self.p_losses_test(batch, dec_use_common_genes = dec_use_common_genes, return_attn_weights = True, attn_layer = self.attn_layer)
            if batch_idx == 0:
                gene_token_embs = output_dict['gene_token_embs'].detach().cpu().numpy()
                self.logger.save_array(gene_token_embs, 'gene_token_embs')


            
            cell_id, celltype_column, attn_weights, gene_out = batch['id'], batch['celltype_code'].detach(), output_dict['attn_weights'], output_dict['gene_out']
            X_samples, X_sample_cell_emb = output_dict['recon_output'].detach(), output_dict['cell_emb'].detach()

            if attn_weights is not None:
                gene_out_mean = torch.mean(gene_out, dim = -1)
                
                
                
                def transfer_to_sparse(dense_matrix):
                    # Set threshold for dropping small values
                    threshold = 1e-3
                    print(f'attn_weights shape: {dense_matrix.shape}')
                    print(f'dense_matrix max: {dense_matrix.max()}')
                    sparse_matrix = sp.csr_matrix(dense_matrix.reshape(-1, dense_matrix.shape[-1]).astype(np.float32), dtype=np.float32)
                    sparse_matrix.data[np.abs(sparse_matrix.data) < threshold] = 0
                    sparse_matrix.eliminate_zeros()
                    return sparse_matrix
    
                file_path = Path(self.logger._save_dir) / f'attn_score_DecL{self.attn_layer}' / f'batch_idx{batch_idx}_ds_idx{dataloader_idx}_rank{self.global_rank}.parquet'
                if not file_path.exists():
                    # tokenized_data = {"id": [], "cell_type": [], "attn_weights": []}
                    tokenized_data = {"id": [], "cell_type": [], "row": [],  "col": [],  "val": [], 'gene_out': []}
        
                    for i in range(len(cell_id)):  # ~2s/100k cells
                        tokenized_data["id"].append(cell_id[i].item())
                        tokenized_data["cell_type"].append(celltype_column[i].item())
                        tokenized_data["gene_out"].append(gene_out[i])
                        
                        sparse_attn = transfer_to_sparse(attn_weights[i].detach().cpu().numpy())
                        c = sparse_attn.tocoo()                 
        
                        tokenized_data["row"].append(c.row)
                        tokenized_data["col"].append(c.col)
                        tokenized_data["val"].append(c.data)
                        
        
        
                    raw_dataset = Dataset.from_dict(tokenized_data)
                    self.logger.save_parquet(raw_dataset, f'batch_idx{batch_idx}_ds_idx{dataloader_idx}_rank{self.global_rank}', folder_name = f'attn_score_DecL{self.attn_layer}')
                    self.logger.log_info(f"Saved raw_dataset batch_idx{batch_idx}_ds_idx{dataloader_idx}_rank{self.global_rank} to {self.logger._save_dir}.")
                    
                
                del attn_weights
                del gene_out
                
                out_dict = {'cell_id': cell_id, 'celltype_column': celltype_column, 'sample_cells': X_samples, 'sample_cell_embs': X_sample_cell_emb, 'gene_out': gene_out_mean}
            else:
                out_dict = {'cell_id': cell_id, 'celltype_column': celltype_column, 'sample_cells': X_samples, 'sample_cell_embs': X_sample_cell_emb, 'gene_out': X_samples}
    ...
    def on_validation_batch_end(self, batch_parts, batch, batch_idx, dataloader_idx = 0): # batch_parts already combined the results of all gpus!! great
        if not self.attn_mode:
            ...
            if dataloader_idx == 0: # out_of_sample
                self.cell_id_out_of_sample.extend(batch_parts['cell_id'].tolist())  
                self.celltype_column_out_of_sample.extend(batch_parts['celltype_column'].tolist())
                self.gene_out_out_of_sample.extend(batch_parts['gene_out'].tolist())  
                self.sample_cells_out_of_sample.extend(batch_parts['sample_cells'].tolist())
                self.sample_cell_embs_out_of_sample.extend(batch_parts['sample_cell_embs'].tolist())
            ...
            if dataloader_idx == 1: # test
                self.cell_id_test.extend(batch_parts['cell_id'].tolist())  
                self.celltype_column_test.extend(batch_parts['celltype_column'].tolist()) 
                self.gene_out_test.extend(batch_parts['gene_out'].tolist())
                self.sample_cells_test.extend(batch_parts['sample_cells'].tolist())
                self.sample_cell_embs_test.extend(batch_parts['sample_cell_embs'].tolist())
    ...
    def on_validation_epoch_end(self): 
        ...
        # if self.attn_mode:
        #     self.attn_weights = torch.stack(self.attn_weights, dim = 0).to(self.device)

            
        #     tokenized_data = {"id": [], "cell_type": [], "attn_weights": []}

        #     data = self.attn_weights
        ...
        #             tokenized_data["id"] = self.cell_id_out_of_sample[i].item()
        #             tokenized_data["cell_type"] = self.celltype_column_out_of_sample[i].item()
        #             tokenized_data["attn_weights"] = self.attn_weights[i].detach().cpu().numpy()
        #             pbar.update(1)
        ...
        #     self.logger.save_parquet(raw_dataset, f'{self.global_rank}_attn_weights')
        #     self.logger.log_info(f"Saved raw_dataset {self.global_rank}_attn_weights to {self.logger._save_dir}.")
        
        #     self.logger.log_info(
        #         f"Dataset number of samples: {len(tokenized_data['id'])}, "
        #         f"\n\t attn_weights shape: {tokenized_data['attn_weights'][0].shape}"
        #     )
        ...
        #     # self.logger.save_array(celltype_column.detach().cpu().numpy(), f'{self.global_rank}_celltype')
        #     # self.logger.save_array(cell_id.detach().cpu().numpy(), f'{self.global_rank}_cell_id')
        #     # self.logger.save_array(attn_weights.detach().cpu().numpy(), f'{self.global_rank}_attn_weights')
            
        #     # self.logger.log_info(f'Writing {self.global_rank}_celltype.npy in {self.logger._save_dir}.')
        #     # self.logger.log_info(f'Writing {self.global_rank}_cell_id.npy in {self.logger._save_dir}.')
        #     # self.logger.log_info(f'Writing {self.global_rank}_attn_weights.npy in {self.logger._save_dir}.')
        ...
        self.gene_out_out_of_sample = torch.Tensor(self.gene_out_out_of_sample).to(self.device)
        ...
        if len(self.CellTypeMapping_df) == 2:
            ...
            self.gene_out_test = torch.Tensor(self.gene_out_test).to(self.device)
        ...
        def plot_cell_sample(cell_id, celltype_column, sample_cells, sample_cell_embs, CellTypeMapping_df, gene_out = None, dataset_idx = None, suffix = None, background_adata = None, testDS = False):
            ...
            gene_out = torch.Tensor(gene_out).cpu().numpy()
            gene_out_adata = sc.AnnData(X = gene_out)
            gene_out_adata.X = csr_matrix(gene_out_adata.X)
            gene_out_adata.obs['cell_type'] = celltype_column
            gene_out_adata.obs['cell_id'] = cell_id
            gene_out_adata.obs['dataset_idx'] = ['dataset_' + str(_) for _ in dataset_idx]
            self.logger.save_h5ad(gene_out_adata, f'{self.current_epoch}_gene_out_adata_{suffix}')
            self.logger.log_info(f'Writing {self.current_epoch}_gene_out_adata.h5ad in {self.logger._save_dir}.')
            self.logger.log_info(f'gene_out_adata shape: {gene_out_adata.shape}.')
            ...
            if testDS:
                if (self.adata_reorder_index is not None) & (background_adata is not None):
        ...
        plot_cell_sample(
            ...
            gene_out = self.gene_out_out_of_sample,
            ...
            testDS = self.valid_use_common_gene
        )
        ...
        if len(self.CellTypeMapping_df) == 2:
            plot_cell_sample(
                ...
                gene_out = self.gene_out_test,
                dataset_idx = self.dataset_idx_test,  
                suffix = 'test',
                background_adata = self.background_adatas[1],
                testDS = True
            )
    ...
    def p_losses_test(self, batch, dec_use_common_genes = False, return_attn_weights = False, attn_layer = 0):
    ...
    def p_losses(self, batch):
        ...
        loss = loss_recon = criterion(output_values, target_values, mask)
        ...
        if self.add_gene_loss:
            ...
            if 'dec_expr' in data_dict.keys():
                ...
                loss_geneloss = criterion_gene(gene_expr_output[mask], target_values[mask])
    ...
    def forward(
        ...
        output = {}
        output['gene_token_embs'] = self.cur_gene_token_embs
        #======================== gene expr decode ===========================#
        if self.decode_gene:
    ...
    #         KL(N(\mu, \sigma), N(0, 1)) = \log \frac{1}{\sigma} + \frac{\sigma^2 + \mu^2}{2} - \frac{1}{2}
        ...
        if (self.mvc_decoder_type is None) or (self.mvc_decoder_type == 'Inner'):
            ...
            if self.attn_layer >= 0:
                dec_output, attn_weights, gene_out = self.mvc_decoder(dec_output, src_key_padding_mask = src_key_padding_mask, attn_layer = attn_layer)
            else:
                dec_output = self.mvc_decoder(dec_output, src_key_padding_mask = src_key_padding_mask)
                attn_weights, gene_out = None, None
            dec_output = self.mvc_expr_decoder(dec_output)
            if return_attn_weights:
                output["attn_weights"] = attn_weights # shape: b x head x sq x sq, batch_size x head x seq_len x seq_len # because of memory constrain, restrict to one dec_layer
                output['gene_out'] = gene_out
