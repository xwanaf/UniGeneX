...
from .preprocess import binning, binning_from_table
...
class DataCollator:
    ...
    data_style: str = "pcpt"
    CLS: bool = False
    binning_from_table: bool = False
    bin_index_to_expr_val_table: np.ndarray = None
    cell_emb_mode: bool = False
    ...
    def _call_pcpt(
        ...
        for i in range(len(examples)):
            ...
            if self.do_binning:
                if self.binning_from_table:
                    expressions[self.keep_first_n_tokens :] = binning_from_table(
                    row=expressions[self.keep_first_n_tokens :],
                    bin_index_to_expr_val_table=self.bin_index_to_expr_val_table,
                )
                else:
            ...
            genes, expressions = self._sample_or_truncate_plus_pad(
                genes, expressions, _max_length, self.cell_emb_mode
        ...
        if self.CLS:
            celltypes = torch.stack(celltypes, dim=0).to(device)

            data_dict = {
                "gene": padded_genes,
                "expr": padded_expressions,
                'celltype_code': celltypes
            }
        else:
            data_dict = {
                "gene": padded_genes,
        ...
        other_keys = [k for k in examples[0].keys() if k not in ['genes', 'expressions', 'celltype_code', 'celltypes']]
        for k in other_keys:
            try:
                data_dict[k] = torch.Tensor([example[k] for example in examples])
            except:
                data_dict[k] = torch.stack([example[k] for example in examples], dim = 0)
        ...
        #============================== mask remove ================================#
        if self.do_mlm:
            masked_genes, masked_expressions = self._mask_remove(
                padded_genes, padded_expressions, self.keep_first_n_tokens
            )
        else:
            masked_expressions = padded_expressions
            masked_genes = padded_genes
        data_dict["masked_gene"] = masked_genes
        data_dict["masked_expr"] = masked_expressions
        #==============================================================#
    ...
    def _call_gen(
        ...
        for i in range(len(examples)):
            ...
            expressions = examples[i]["expressions"]
            if self.CLS:
                celltype = torch.Tensor(examples[i]["celltypes"])
            if self.do_binning:
                if self.binning_from_table:
                    expressions[self.keep_first_n_tokens :] = binning_from_table(
                    row=expressions[self.keep_first_n_tokens :],
    ...
    def _mask_multiMaskRatio(
        self, genes: torch.Tensor, expressions: torch.Tensor, keep_first_n_tokens: int = 0
    ) -> torch.Tensor:
        """
        Mask the expression values with MLM.
        """
        if keep_first_n_tokens > 0:
            result_gene_, result_expression_ = self._mask_multiMaskRatio(
                genes[:, keep_first_n_tokens:],
                expressions[:, keep_first_n_tokens:],
                keep_first_n_tokens=0,
            )
            return torch.cat([genes[:, :keep_first_n_tokens], result_gene_], dim=1), torch.cat([expressions[:, :keep_first_n_tokens], result_expression_], dim=1)
        ...
        masked_genes = genes.masked_fill(mask, self.pad_token_id)
        masked_expressions = expressions.masked_fill(mask, self.pad_value)
        
        return masked_genes, masked_expressions
    ...
    def _mask_remove(
        ...
        """
        if keep_first_n_tokens > 0:
            result_gene_, result_expression_ = self._mask_remove(
                genes[:, keep_first_n_tokens:],
                expressions[:, keep_first_n_tokens:],
                keep_first_n_tokens=0,
            )
        ...
        num_remove_per_row = int(self.get_mlm_probability() * shape[1])
        num_remain_per_row = shape[1] - num_remove_per_row
        # Create a mask matrix with True values
        mask_matrix = np.ones(shape, dtype=bool)
        for row in range(shape[0]):
            false_indices = np.random.choice(shape[1], size=num_remove_per_row, replace=False)
            mask_matrix[row, false_indices] = False
        mask_matrix = torch.from_numpy(mask_matrix)
        mask_matrix = mask_matrix.to(device)

        row_indices, col_indices = np.where(mask_matrix)
        masked_mtx = []
        for mtx in [genes, expressions]:
            filtered_matrix = mtx[row_indices, col_indices]
            filtered_matrix = filtered_matrix.reshape((shape[0], num_remain_per_row))
            masked_mtx.append(filtered_matrix)
        masked_genes, masked_expressions = masked_mtx
