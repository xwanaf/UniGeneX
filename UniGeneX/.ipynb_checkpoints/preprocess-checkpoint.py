...
def binning_from_table(
    row: Union[np.ndarray, torch.Tensor], bin_index_to_expr_val_table: Union[np.ndarray, torch.Tensor]
) -> Union[np.ndarray, torch.Tensor]:
    """Binning the row into n_bins."""
    dtype = row.dtype
    return_np = False if isinstance(row, torch.Tensor) else True
    row = row.cpu().numpy() if isinstance(row, torch.Tensor) else row
    bin_index_to_expr_val_table = bin_index_to_expr_val_table.cpu().numpy() if isinstance(bin_index_to_expr_val_table, torch.Tensor) else bin_index_to_expr_val_table
    # TODO: use torch.quantile and torch.bucketize
...
#         bins = np.quantile(non_zero_row, np.linspace(0, 1, n_bins - 1))
    ...
    bins = bin_index_to_expr_val_table
    non_zero_digits = _digitize(non_zero_row, bins)
    binned_row = np.zeros_like(row, dtype=np.int64)
    binned_row[non_zero_ids] = non_zero_digits
    return torch.from_numpy(binned_row) if not return_np else binned_row.astype(dtype)
