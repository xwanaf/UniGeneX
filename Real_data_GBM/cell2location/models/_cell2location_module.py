...
class LocationModelLinearDependentWMultiExperimentLocationBackgroundNormLevelGeneAlphaPyroModel(PyroModule):
    ...
    def __init__(
        ...
        if self.nuclei_segmentation:
            self.register_buffer("n_s_cells_per_location", torch.Tensor(n_s_cells_per_location))
        self.use_bg = use_bg
        print(f' ################################### ')
        print(f'use_bg: {self.use_bg}')
        print(f' ################################### ')
        if self.use_bg and (init_bg_mean is not None):
            self.register_buffer("init_bg_mean", torch.Tensor(init_bg_mean))
        ...
        if self.nuclei_segmentation:
            with obs_plate as ind:
                n_s_cells_per_location = pyro.sample(
                    k,
                    dist.Gamma(
                        self.n_s_cells_per_location.unsqueeze(1),
                        torch.ones(1, device=self.n_s_cells_per_location.device).unsqueeze(1),
    #                     self.ones
                    ).to_event(0),
                )
        else:
        ...
        if self.use_bg:
            ...
            s_g_gene_add = s_g_gene_add * self.init_bg_mean
        else:
            s_g_gene_add = pyro.sample(
                "s_g_gene_add",
                dist.Gamma(
